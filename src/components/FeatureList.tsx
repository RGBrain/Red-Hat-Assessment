...
const FeatureList: React.FC<{ listData: FeatureItem[] }> = ({ listData }) => {
  return (
    <div className="mx-auto mt-0 flex max-w-[1400px] flex-col space-y-24 md:flex-row md:flex-wrap md:justify-center md:space-y-0 md:gap-x-20 md:gap-y-30 xl:gap-x-16">
      {listData.map(
        (
          { title, description, titleClass, descriptionClass, icon: Icon, iconClass, iconSize },
          idx,
        ) => (
          <div
            key={title ?? idx}
          ...
          >
            ...
            <p
              className={`mt-3 text-[1.10rem] leading-6 text-black md:text-base md:leading-6 xl:px-4 xl:text-[1.20rem] xl:leading-[1.85rem] xl:font-normal ${descriptionClass ?? ''}`}
            >
              {description}
            </p>
          </div>
